...
describe("focused SQLite Grant authority boundary", () => {
  ...
  test("keeps migration-006 Grants authoritative and rotatable", async () => {
    ...
    try {
      ...
      try {
        ...
        expect(await current.readiness()).toMatchObject({ ok: true, schemaVersion: "19" });
  ...
  test("freezes SQLite after previous Event Game audit provenance tampering", async () => {
    ...
    try {
      ...
      expect(await restarted.readiness()).toMatchObject({ ok: true, schemaVersion: "19" });
      restarted.close();
  ...
  test("rejects replay self-links and active origins across SQLite restart validation", async () => {
    ...
    try {
      ...
      expect(await readyRestart.readiness()).toMatchObject({ ok: true, schemaVersion: "19" });
      readyRestart.close();
      ...
      expect(await repairedRestart.readiness()).toMatchObject({ ok: true, schemaVersion: "19" });
