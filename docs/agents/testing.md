...
### Issue #97 composed live-control convergence coverage

The fixed-seed 1,000-sequence composed Live Event Game control proof remains a Fast Test: every sequence starts with fresh in-memory Event Game and Control Grant state, uses two admitted Controllers, injected clocks, and the actual acceptance, record, and replay seams without external processes, network traffic, Production data, or production-shaped load. On Darwin arm64 with Bun 1.3.14, the exact command `bun test --isolate ./src/lib/live-event-game-control.test.ts --test-name-pattern 'audits the complete live-control noncommutative matrix through composed replay'` measured 16,135.37 ms on run 1 and 16,207.75 ms on run 2 on 2026-08-15. The obvious simple optimizations reused the Controller Session already admitted by each fresh harness instead of admitting a redundant third session, and made generated batches yield cooperatively so their CPU work does not starve other Fast Test deadlines; the proof remains materially above the per-file review target while retaining the required composed coverage. CPU, memory, and other resource telemetry were unavailable: Bun's ordinary test output exposes durations but not resource counters, and no external profiler or production-shaped workload was authorized for this correction.

The test retains all 1,000 fresh composed multi-Controller sequences because reusing converged fixtures, reducing the sequence count, or substituting semantic rebuild-only fixtures would no longer establish issue #97's required acceptance and replay behavior. Its 60-second test deadline is a bound, not the timing decision; two Linux deployment runs reached the former 30-second deadline under concurrent ordinary-suite load on 2026-08-15, while the unchanged targeted proof completed locally in about 15–16 seconds. The complete ordinary suite also breached the ten-second suite review target: the exact command `bun run test` measured 73.62 seconds for 906 passing tests on the same Darwin arm64/Bun 1.3.14 environment. That total includes the retained 1,000-sequence composed proof and the repository's other ordinary Fast Tests, but no external processes, network traffic, Production data, or qualification workload; the temporary review budget is 75 seconds for this final-base baseline. Under the two-day minimum-reliable policy, revisit if either the composed proof exceeds 18 seconds or the ordinary suite exceeds 75 seconds in three consecutive native runs on each of two separate days, or immediately if any run exceeds 90 seconds. Revisit sooner if a simpler composed fixture removes material setup cost without weakening fresh state, multi-Controller participation, or the production acceptance/record/replay seams.
