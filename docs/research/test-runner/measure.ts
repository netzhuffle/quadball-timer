...
await mkdir(output); // Refuse old tables rather than silently reusing scheduling data.
const metadata = {
  bun: Bun.version,
  ...
  runner: process.env.RUNNER_NAME ?? "native",
...
async function run(name: string, workers: number, scheduled: boolean) {
  ...
  const deadline = setTimeout(() => {
    boundedStop = true;
    stopGroup("SIGTERM");
    forceStop = setTimeout(() => stopGroup("SIGKILL"), 5_000);
  }, 240_000);
  const interrupted = () => {
    boundedStop = true;
    stopGroup("SIGTERM");
  ...
  if (boundedStop) stopGroup("SIGKILL");
  const exitCode = boundedStop ? 124 : childExitCode;
  const result = {
    name,
    workers,
    scheduled,
...
for (let round = 0; round < 3; round++) {
  ...
  for (const [workers, scheduled] of order) {
    if (
      await run(
        `r${round + 1}-w${workers}-${scheduled ? "timings" : "discovery"}`,
        workers,
        scheduled,
      )
    )
